...
public class VerifyCodeUtil {
	...
	public static BufferedImage generateImageCode(int type, int length, String exclude, int width, int height, int interLine, boolean randomLocation, Color backColor, Color foreColor, Color lineColor){
		...
		switch(type){
			case TYPE_NUM_ONLY:
				while(i < length){
					...
					if(null == exclude || exclude.indexOf(t+"")<0){
						verifyCode.append(t);
						i++;
					}
				}
			...
			case TYPE_LETTER_ONLY:
				while(i<length){
					...
					if((t>=97 || (t>=65&&t<=90)) && (null==exclude || exclude.indexOf((char)t)<0)){
			...
			case TYPE_ALL_MIXED:
				while(i < length){
					int t = random.nextInt(123);
					if((t>=97 || (t>=65&&t<=90) || (t>=48&&t<=57)) && (null==exclude||exclude.indexOf((char)t)<0)){  
                        verifyCode.append((char)t);  
                        i++;  
                    } 
				}
			...
			case TYPE_NUM_UPPER:
				while(i < length){  
                    ...
                    if((t>=65 || (t>=48&&t<=57)) && (null==exclude || exclude.indexOf((char)t)<0)){  
                        verifyCode.append((char)t);  
                        i++;  
                    }  
                }  
			...
			case TYPE_NUM_LOWER:  
                while(i < length){  
                    int t = random.nextInt(123);  
                    if((t>=97 || (t>=48&&t<=57)) && (null==exclude || exclude.indexOf((char)t)<0)){  
                        verifyCode.append((char)t);  
                        i++;  
                    }  
                }  
            ...
            case TYPE_UPPER_ONLY:  
                while(i < length){  
                    int t = random.nextInt(91);  
                    if((t >= 65) && (null==exclude||exclude.indexOf((char)t)<0)){  
                        verifyCode.append((char)t);  
                        i++;  
                    }  
                }  
            ...
            case TYPE_LOWER_ONLY:  
                while(i < length){  
                    int t = random.nextInt(123);  
                    if((t>=97) && (null==exclude||exclude.indexOf((char)t)<0)){  
                        verifyCode.append((char)t);  
                        i++;  
                    }  
                }  
	...
	public static void main(String[] args){
		...
		try {
			ImageIO.write(bufferedImage, "JPEG", new File("D:\\test.jpeg"));
		} catch (IOException e) {
